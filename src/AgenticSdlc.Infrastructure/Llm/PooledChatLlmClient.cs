// AgenticSdlc.Infrastructure/Llm/PooledChatLlmClient.cs
// SDK-based provider client: a keyed pool of Microsoft.Extensions.AI IChatClient instances (one per API
// key) selected by ApiKeyRouter, with round-robin + rate-limit (429) failover. Provider-agnostic — the
// key->IChatClient factory and the "is this a rate-limit error" predicate are injected, so the same class
// serves Claude (Anthropic.SDK) and Azure OpenAI (Azure.AI.OpenAI). Replaces the hand-rolled raw clients.
...
/// <summary>
/// <see cref="ILlmClient"/> backed by a pool of <see cref="IChatClient"/> instances keyed by API key.
/// Picks a key via <see cref="ApiKeyRouter"/>, sends through that key's (cached) chat client, and on a
/// rate-limit error cools the key down and retries on the next — full failover across the pool.
/// </summary>
public sealed class PooledChatLlmClient : ILlmClient
{
    ...
    /// <summary>Builds the pooled client.</summary>
    /// <param name="provider">Provider tag for responses / router partition.</param>
    /// <param name="clientFactory">Creates an <see cref="IChatClient"/> for a given API key (cached per key).</param>
    /// <param name="keyProvider">Returns the current distinct key pool (override + config), resolved per call.</param>
    /// <param name="router">Shared key router (round-robin + cooldown).</param>
    /// <param name="isRateLimited">True when an exception represents a rate-limit (HTTP 429) the pool should fail over on.</param>
    /// <param name="retryAfter">Optional cooldown hint extracted from the exception (provider's Retry-After).</param>
    /// <param name="logger">Logger.</param>
    /// <param name="baseDelay">Backoff base between failover attempts (default 1s).</param>
    ...
    public async Task<LlmResponse> SendAsync(LlmRequest request, CancellationToken cancellationToken = default)
    {
        ...
        // At least one attempt per key, so the pool can fully fail over on repeated rate-limits.
        ...
        for (var attempt = 0; attempt < maxAttempts; attempt++)
        {
            ...
            var chat = _clients.GetOrAdd(key, _clientFactory);
