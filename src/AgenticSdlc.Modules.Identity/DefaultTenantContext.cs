// Single-operator fallback ITenantContext (Auth:Mode=operator). Everything maps to the "default"
// tenant with the admin role, so pre-multi-tenant behavior is preserved. Replaced by a claims-based
// HttpTenantContext (read from the Keycloak token) when Auth:Mode=keycloak.
