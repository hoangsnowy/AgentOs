...
**Design workflow — Claude Design is upstream; pull DOWN into the repo (one direction).** The visual language is authored in the **Claude Design** project [AgentOS Design System](https://claude.ai/design/p/bec4d21d-0fad-4d66-9abe-ed8c711b75c8) (Published + org Default). Direction: **GNOME / Adwaita**. The canonical loop is:
...
4. **Verify E2E** in the standalone Web (light + dark) and **commit**.

Hand-editing tokens/chrome directly in the repo is a **fallback only** (Claude Design unavailable / auth not set up). When you do, you MUST push the same change back up to the Claude Design project so the two stay in sync — the project is the source of truth, the repo is the pulled artifact. Keep branding **AgentOS**, one accent, responsive (≤1024 = full-screen sheets).
